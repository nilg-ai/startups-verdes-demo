...
function Directions({
      ...
      fetch(
        ...
        .then((res) => {
          if (res.STATUS === 'SUCCESS') {
            ...
            setRoutes([res.DATA[0].best_route, ...res.DATA[0].other_routes]);
  ...
  return (
    <section>
      ...
      {routes.length > 0 ? (
        selectedRoute ? (
          <RouteDetails route={selectedRoute} backPressed={onBackClick} />
        ) : (
          <AvailableRoutes routes={routes} routeClick={onRouteClick} />
        )
      ) : (
        <></>
      )}
